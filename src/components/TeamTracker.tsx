...
import { resolveTeamRecord, usePlayoffs, useStandings, getPlayerPoints, getTeamPoints } from '../lib/standings';

export function TeamTracker() {
  const player = getCurrentPlayer();
  const { standings } = useStandings();
  const { playoffs } = usePlayoffs();
  const totalPoints = getPlayerPoints(player, standings, playoffs);
  
  // Sort teams by wins (best to worst)
  const sortedTeams = [...player.teams].sort(
    (a, b) =>
      resolveTeamRecord(b, standings).wins - resolveTeamRecord(a, standings).wins,
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-fuchsia-50 rounded-xl p-5 shadow-sm border border-fuchsia-200">
        ...
        <div className="text-4xl text-gray-900 mb-3">{totalPoints}</div>
        <div className="flex items-center gap-4 text-sm text-gray-700">
          <div>
            <span className="text-gray-600">Games Played:</span>
            <span className="ml-2 font-medium">
              {player.teams.reduce(
                (sum, t) => sum + resolveTeamRecord(t, standings).gamesPlayed,
                0,
              )}
            </span>
          </div>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-gray-500 mt-0.5">{teamInfo.division}</div>
                </div>
            ...
            {/* Stats Bar */}
              <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-gray-200">
                <div>
                  <div className="text-lg text-fuchsia-600">
                    {getTeamPoints(team.teamId, standings, playoffs)}
                  </div>
                  <div className="text-xs text-gray-500">Points</div>
                </div>
                <div>
                  <div className="text-lg text-gray-900">{17 - record.gamesPlayed}</div>
