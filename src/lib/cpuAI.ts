// CPU manager AI (draft-engine-spec.md §4). Pure valuation + sampling logic —
// no knowledge of draft state persistence, so it's easy to unit test and to
// reuse from both interactive CPU turns and bulk draft simulations.
...
/** §4.3 — the homer bonus only ever applies inside this manager's own valuation. */
...
/**
 * §4.1 — score a candidate team under one of the three strategies.
...
/**
 * §4.2 — sample from a softmax over the manager's own scores rather than
 * argmax'ing. TAU=1.0 self-scales: a team 2 points below the top draws ~14%
 * relative weight, 5 points below draws under 1%.
 */
